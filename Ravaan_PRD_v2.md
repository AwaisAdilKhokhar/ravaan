...
| ↳ **verdict, 2026-09-13** | | **SPLIT.** Resume: **PASS** on both arms — 5,614 steps, 367,919,104 tokens each, reload asserted parameter-identical. Coherence: **FAIL** — Ravaan-AR is fluent, Ravaan-DIFF is not, under any of 16 decoder settings (`reports/pilot_coherence.md`, 306 generations) | **Fallback not yet taken — it may not be the right one.** This row's kill reads "implementation bug", and no implementation bug was found: both arms train, checkpoint, resume exactly and decode, and three separate decoder defects were found *and fixed* without changing the verdict. The pilot is 25M parameters at 368M training tokens over a 7.36M-token corpus, which may simply be below the scale at which "coherent Urdu" is reachable — in which case the gate's premise is wrong rather than the code. **Open: debug further, re-pilot at a larger rung, or record the gate as unmet on its own terms and proceed to W9** |
