...
| **Ablations A3/A4** — inference only | §4.4 | ✅ **s32, re-swept s36 on the released checkpoint** — `ship-diff-b/diff-s0_f1.pt` then `ship-diff-b64/diff-s0_f1_weights.pt`, 300 generations each → **Findings BG, BU**. A3 runs backwards on both, so AP replicates at 70M/85.4M at 16 *and* 64 epochs. ⚠️ **A4 no longer has a winner**: `random` ties `gumbel 2` on script consistency at 64 epochs and leads distinct-1, so the schedule is an open question a fluent reader settles, not a measured default |
...
| AP | 22 | A3 runs backwards — 64 denoising steps repeat *more* than 8, monotonically on every axis | ~~⚠️ directional, one seed~~ → ✅ **replicated twice at full scale — s32 (Finding BG) and s36 (Finding BU)**: 8 steps beat 160 on a 70M checkpoint over 85.4M tokens at 16 epochs and again at 64, same direction all three times. **A3's table must not be written as "more steps, better"** |
...
3. ✅ **Re-run the A3/A4 sweep on the *released* checkpoint — done 2026-09-24 (Finding BU).**
   ...
   default was right; **A4 did not** — `random` and `gumbel 2` are now tied on the metrics, and
   the demo shows both rather than asserting a default the evidence no longer supports. The
   defaults in `release_assets/generate.py` and both model cards are **unchanged on purpose**;
   they should change only when a fluent reader settles the schedule.

   ⚠️ **The successor to this item is the axis nobody has swept**: `temperature` and `top_p` have
   been pinned at 0.9/0.95 in every sweep this project has run. `sample_ids` returns the
   post-filter probability of the *sampled* token, and `sample_diffusion` ranks commit order by
   that same number — so temperature reshapes **which positions get committed first** on the
   diffusion arm and only reshapes token choice on AR's. ~25 min on the 4060, $0.

   ```bash
   python -u scripts/sample.py --checkpoint D:/ravaan-runs/ship-diff-b64/diff-s0_f1_weights.pt \
       --corpus data/packed --tokenizer data/tokenizer/ravaan-16k.model \
       --out reports/sweep_temp --samples 5 --new-tokens 160 --forbid-eos always \
       --steps 8 --schedule gumbel --schedule random --gumbel 2 --temperature 0.7
   ```
...
# the demo itself (s36): AR's best against the RELEASED diffusion checkpoint, both schedules.
# Matched corpus arm - do NOT reach for core-diff-s1 here, that was the arm-A mistake (Finding BU)
python -u scripts/sample.py --checkpoint D:/ravaan-runs/ship-ar-b/ar-s0_fp25.pt \
    --checkpoint D:/ravaan-runs/ship-diff-b64/diff-s0_f1_weights.pt \
    --corpus data/packed --tokenizer data/tokenizer/ravaan-16k.model \
    --out reports/demo_b64 --samples 12 --new-tokens 160 --forbid-eos always \
    --steps 8 --schedule gumbel --schedule random --gumbel 2
# ~9 min on the 4060 to index 85.4M tokens; gates the page's non-recitation claim
python scripts/overlap.py --samples reports/demo_b64.jsonl --corpus data/packed \
    --corpus-arm B --verify --out reports/eval/overlap_demo_b64.json
python scripts/demo_page.py --samples reports/demo_b64.jsonl --out reports/demo

# the hosted decoder demo (s36): traces BOTH schedules from one seed, then rebuilds site/
# ~4 min on the 4060. Needs D:/ravaan-release, not the training pickles - it animates what
# someone who pip-installs actually gets. Pushing site/ to main redeploys Pages.
python -u scripts/demo_trace.py
python scripts/decoder_demo.py --site
