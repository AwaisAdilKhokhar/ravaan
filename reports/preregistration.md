...
| 2026-09-14 | **Ablation A4 gains a third arm and A3 gains a rung, both inference-only.** A4 is measured as `random` / `confidence` / `gumbel(s)`, where the third ranks positions by `log p(chosen) + s·Gumbel(0,1)` with *s* annealed to zero over the decode, swept at s ∈ {1, 2}. A3 is measured at 8/16/32/64 **and at one position per step**, which for a canvas of *n* masked positions is *n* steps. Nothing else moves: no training-side change, no endpoint change, and P1, P3 and P4 are unaffected. The two original A4 settings and the four original A3 rungs are still measured and still reported. | **A4's two settings are the two limits of one dial rather than two options, and the grid contained neither the dial nor its interior.** `gumbel` at s = 0 reproduces `confidence` *exactly* (`log` is monotone; asserted in `tests/test_sampling.py`, not claimed) and as s → ∞ reproduces `random`. Measured on the pilot diffusion checkpoint, both endpoints fail and they fail in opposite directions — `confidence` collapses to one clause (rep up to 0.69), `random` produces non-repetitive **non-words** — while the interior produces real Urdu words in grammatical clauses from the same weights. A3 has the same defect at its ceiling: 64 steps on a 160-position canvas still commits 2–3 positions per step from independent marginals, so the grid never contained the exact any-order ancestral sampler it was implicitly being compared against. **This is a decoder change and no model, objective, corpus or endpoint moves with it**; it is logged here because §4.4's ablation grid was fixed at preregistration and an addition to it is a deviation even when it only adds. **No model in the reported experiment has been trained and no result has been seen** — the evidence is the Week 8 pilot, which is not in the results table (PRD §11 G3). See `reports/pilot_coherence.md` §10. |
| 2026-09-16 | **Seeds 2 and 3 are dropped. The project trains two models, one seed each — one AR, one diffusion.** §4's primary endpoint is no longer *"aggregated over 3 seeds"*: it becomes a **single paired comparison** of two curves. **Ablations A1 and A2 are withdrawn** (each cost a training run); A3 and A4 are inference-only and stand unchanged, as does everything in §4's secondary family. **P1, P3 and P4 stand as written** — no prediction is restated, withdrawn or re-scoped — but the *evidence that will resolve them* is now one run per arm, and the report must say so where each is resolved: P1's crossing is read off one pair of curves, P3's location off that same pair, and P4's fitted half-lives come from one run each, so their intervals are checkpoint-level and carry no between-seed component. | **Schedule, and — unlike every earlier row in this log — not forced by a measurement.** The frozen corpus supports the six runs PRD v2.3 planned; the project owner has set time-to-a-reported-result as the priority and taken PRD §10's pre-committed minimum viable cut, which names *ablations A1/A2* and then *seeds 2 and 3* as the last things to drop and judges "the epoch sweep on a single seed for both models" the irreducible core. **The integrity condition this log exists to protect is intact: no model in the reported experiment has been trained and no result has been seen.** The training corpus did not exist when this was written — stage 10 had not yet packed it — so the change cannot have been informed by any outcome. **What it costs, and the report must state it beside the primary result rather than in a closing limitations section:** with one run per arm there is no estimate of initialization variance, so a measured AR/DIFF gap **cannot be separated from a single seed draw**. §4's paired bootstrap still runs and still resamples *eval items*, which is a different quantity — an item-level CI excluding zero does not show the sign would survive a different seed. Losing A2 additionally withdraws the claim that quantified how much an un-equipped AR baseline inflates the comparison **on Urdu**; MARIA (arXiv:2502.06901) is cited for the effect's existence and Ravaan states it did not measure the size. See PRD §0.4. |
| 2026-09-20 | **U is 23,214,080 unique tokens, not the 25M named in §2 and the appendix, and the realized population mixture is 68.8 / 26.1 / 5.1 against §6.1's 70.6 / 23.5 / 5.9.** The epoch count moves ~396 → **~426** at the same 9.9B tokens processed. **C_crit moves with U**: the appendix's C_crit(25M) = 2.32 × 10¹⁸ FLOPs becomes **C_crit(23.21M) = 1.973 × 10¹⁸**, and arm A's compute of 4.153 × 10¹⁸ therefore sits **2.11× past it rather than 1.79×** (`scripts/crossover.py --params 70e6 --unique 23214080 --epochs 426`). **P1, P3 and P4 stand as written and none is re-scoped.** P3's window is read against the corrected C_crit, and a 7% move in U is far inside its one-order-of-magnitude band. The report must quote the realized U, epoch count and mixture, not the planned ones. | **A measurement at the freeze, not a choice, and it does not weaken the design — it strengthens it.** Arm A's cut is a fixed bucket range solved on stage 9's *pre*-stage-8 histogram; stage 8 then removed 13.5% of the characters inside those buckets, so the cut delivered 92.9% of its token budget. The pool is not short — urdu alone still holds ~2.7B clean tokens — and `roman_urdu` came in *over* target at 103.0% because only 592 of its rows were removed, which is what tilted the mixture toward it. **Less unique data at the same compute puts the model further into the repeat regime, which is the regime §4.3 exists to test**, and the fitted law still places the crossover at U = 33M, so arm A at 23.21M remains on the side where P1 predicts a cross — more clearly than at 25M. Fixing it exactly would mean re-measuring stage 9 over the post-stage-8 corpus and re-solving the arm cuts with the held-out bands held fixed (so the 40-hour stage 8 pass stays valid) and re-packing, ~12 h plus new code on the critical path; it was declined against the stated priority of time to a reported result. **No model has been trained and no result has been seen** — the corpus was packed on 2026-09-20 and no run has started. |
